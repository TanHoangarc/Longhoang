...
import { Search, Save, Download, Calculator, Settings, X, Edit3, CheckSquare, Square, ArrowUpDown, Filter, DollarSign, Plus, Trash2, Wand2, User, ShieldCheck, AlertCircle, Briefcase, Calendar, CreditCard } from 'lucide-react';
...
const AccountSalary: React.FC<AccountSalaryProps> = ({ users, attendanceRecords }) => {
      ...
      users.forEach(u => {
          if (!initData[u.id]) {
              ...
              initData[u.id] = {
                  ...
                  companyFund: 50000,
                  familyDeduction: 11000000, // Standard deduction
                  holidayBonus: 0,
                  unionFee: 0,
                  note: '',
  ...
  const applyPresetValue = (field: string, type: MoneyConfigType) => {
      // Find config for current selected month/year
      ...
      if (config) {
          ...
          alert(`Chưa có cấu hình khoản tiền "${newMoneyConfig.name}" cho Tháng ${selectedMonth}/${selectedYear}. Vui lòng tạo trong mục "Cấu hình Thu nhập".`);
      }
  };

  // Helper to count visible sub-columns for colspan
  ...
  const getBankStyle = (bankName: string = '') => {
    ...
    if (name.includes('vp')) return 'bg-gradient-to-br from-green-500 to-teal-600';
    if (name.includes('bidv')) return 'bg-gradient-to-br from-blue-600 to-teal-600';
    if (name.includes('vietin')) return 'bg-gradient-to-br from-blue-600 to-red-600';
    if (name.includes('tp')) return 'bg-gradient-to-br from-purple-600 to-purple-800';
    ...
    <div className="space-y-4 h-full flex flex-col animate-in fade-in duration-300 w-full max-w-full">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white p-4 rounded-xl border border-gray-100 shadow-sm gap-4">
            ...
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                ...
                {/* Settings Toggle - Renamed to Thiết lập */}
        ...
        <div className="bg-white border border-gray-300 shadow-md rounded-lg overflow-x-auto flex-1 w-full relative">
            <table className="w-full border-collapse text-xs min-w-full">
                {/* 
                    FIX: Increased z-index of thead to z-40 so it stays above sticky tbody cells (z-20) 
                    The sticky name column header now has z-50 to stay above everything.
                */}
                <thead className="sticky top-0 z-40 shadow-sm">
                    {/* Header Row 1 - Cream Color Update */}
                    <tr className="bg-[#FFFBF0] text-slate-800 font-bold uppercase tracking-wider text-center h-10 border-b border-orange-200">
                        ...
                        {/* Sticky Name Column - Fix z-index & background */}
                        <th className="border border-orange-200 min-w-[180px] sticky left-0 z-50 bg-[#FFFBF0]" rowSpan={2}>
                            <div className="flex items-center justify-between px-2">
                                <span>Tên Nhân viên</span>
                                <button onClick={handleSortToggle} className="p-1 hover:bg-orange-100 rounded transition" title="Sắp xếp A-Z">
                        ...
                        {getColSpan(['comp_bhxh', 'comp_bhyt', 'comp_bhtn', 'comp_union']) > 0 && (
                            <th className="border border-orange-200 bg-[#f3f4f6]" colSpan={getColSpan(['comp_bhxh', 'comp_bhyt', 'comp_bhtn', 'comp_union'])}>Bảo hiểm & KPCĐ Cty đóng</th>
                        )}
                    ...
                    <tr className="bg-[#FFFBF0] text-slate-700 font-bold uppercase text-[10px] text-center h-10 border-b border-orange-200">
                        ...
                        {/* Cty đóng */}
                        {visibleColumns.comp_bhxh && <th className="border border-orange-200 w-[80px] bg-[#f3f4f6]">BHXH (17.5%)</th>}
                        {visibleColumns.comp_bhyt && <th className="border border-orange-200 w-[80px] bg-[#f3f4f6]">BHYT (3%)</th>}
                        {visibleColumns.comp_bhtn && <th className="border border-orange-200 w-[80px] bg-[#f3f4f6]">BHTN (1%)</th>}
                ...
                <tbody className="text-sm font-medium text-gray-700">
                    {filteredUsers.map((u, idx) => {
                        const d = payrollData[u.id] || { 
                            basic: 0, kpiCurrent: 0, kpiHold: 0, kpiReturn: 0, bonus: 0, salary13: 0, parking: 0,
                            advance: 0, insuranceBase: 0, companyFund: 0, familyDeduction: 0, holidayBonus: 0, unionFee: 0, note: '',
                            isContractSigned: true
                        };

                        // 1. Calculate Work Days from Attendance
                        const standardDays = 26;
                        const workDays = attendanceRecords.filter(r => {
                            const rDate = new Date(r.date);
                            return r.userId === u.id && 
                                   rDate.getMonth() + 1 === selectedMonth && 
                                   rDate.getFullYear() === selectedYear &&
                                   (r.status === 'Present' || r.status === 'Late' || (r.status === 'On Leave' && r.leaveType === 'Paid'));
                        }).reduce((acc, r) => acc + (r.leaveDuration || 1), 0);

                        // 2. Salary by Time
                        const salaryTime = (d.basic / standardDays) * workDays;

                        // 3. Total Income (Tổng số)
                        const effectiveSalary13 = d.isContractSigned ? d.salary13 : 0;
                        const totalIncome = (salaryTime + d.kpiCurrent + d.kpiReturn + d.bonus + effectiveSalary13 + d.parking) - d.kpiHold;

                        // 4. Insurance (Employee) - only if contract signed
                        const bhxh = d.isContractSigned ? d.insuranceBase * 0.08 : 0;
                        const bhyt = d.isContractSigned ? d.insuranceBase * 0.015 : 0;
                        const bhtn = d.isContractSigned ? d.insuranceBase * 0.01 : 0;

                        // 5. Total Taxable Income
                        const taxableIncomeTotal = totalIncome - d.advance + d.holidayBonus;

                        // 6. Assessable Income
                        const totalInsuranceEmp = bhxh + bhyt + bhtn;
                        let assessableIncome = taxableIncomeTotal - totalInsuranceEmp - d.familyDeduction;
                        if (assessableIncome < 0) assessableIncome = 0;

                        // 7. PIT Calculation
                        const pit = calculatePIT(assessableIncome);

                        // 8. Total Deductions
                        const effectiveCompanyFund = d.isContractSigned ? d.companyFund : 0;
                        const totalDeductions = totalInsuranceEmp + effectiveCompanyFund + pit;

                        // 9. Net Salary
                        const netSalary = totalIncome - d.advance - totalDeductions - effectiveSalary13;

                        // 10. Company Cost
                        const compBhxh = d.isContractSigned ? d.insuranceBase * 0.175 : 0;
                        const compBhyt = d.isContractSigned ? d.insuranceBase * 0.03 : 0;
                        const compBhtn = d.isContractSigned ? d.insuranceBase * 0.01 : 0;
                        const effectiveUnionFee = d.isContractSigned ? d.unionFee : 0;
                        ...
                        return (
                            <tr key={u.id} className="hover:bg-yellow-50 transition border-b border-gray-200 group">
                                ...
                                {/* Sticky Name Column: Solid Background + Z-index fixed */}
                                <td 
                                    className="border border-gray-300 px-3 font-bold sticky left-0 bg-white z-20 shadow-sm text-blue-900 cursor-pointer hover:underline hover:text-blue-700 transition-colors"
                                ...
                                {/* KPI */}
                                {visibleColumns.kpi_current && <td className="border border-gray-300 px-2 text-right">{formatMoney(d.kpiCurrent)}</td>}
                                {visibleColumns.kpi_hold && <td className="border border-gray-300 px-2 text-right text-red-500">{formatMoney(d.kpiHold)}</td>}
                                {visibleColumns.kpi_return && <td className="border border-gray-300 px-2 text-right text-green-600">{formatMoney(d.kpiReturn)}</td>}
                                
                                {visibleColumns.bonus && <td className="border border-gray-300 px-2 text-right">{formatMoney(d.bonus)}</td>}
                                {visibleColumns.salary_13 && <td className="border border-gray-300 px-2 text-right">{d.isContractSigned ? formatMoney(d.salary13) : '-'}</td>}
                                {visibleColumns.parking && <td className="border border-gray-300 px-2 text-right">{formatMoney(d.parking)}</td>}
                                ...
                                {/* Insurance Base */}
                                {visibleColumns.insurance_base && <td className="border border-gray-300 px-2 text-right font-bold">{d.isContractSigned ? formatMoney(d.insuranceBase) : '-'}</td>}
                                
                                {/* Deductions */}
                                {visibleColumns.deduct_bhxh && <td className="border border-gray-300 text-right px-2 text-xs">{d.isContractSigned ? formatMoney(bhxh) : '-'}</td>}
                                {visibleColumns.deduct_bhyt && <td className="border border-gray-300 text-right px-2 text-xs">{d.isContractSigned ? formatMoney(bhyt) : '-'}</td>}
                                {visibleColumns.deduct_bhtn && <td className="border border-gray-300 text-right px-2 text-xs">{d.isContractSigned ? formatMoney(bhtn) : '-'}</td>}
                                {visibleColumns.deduct_fund && <td className="border border-gray-300 px-2 text-right text-xs">{d.isContractSigned ? formatMoney(d.companyFund) : '-'}</td>}
                                {visibleColumns.deduct_pit && <td className="border border-gray-300 text-right px-2 font-bold text-orange-600">{formatMoney(pit)}</td>}
                                {visibleColumns.deduct_total && <td className="border border-gray-300 text-right px-2 font-black text-red-700 bg-red-50">{formatMoney(totalDeductions)}</td>}
                                
                                {/* Tax Calculation fields */}
                                {visibleColumns.tax_family && <td className="border border-gray-300 px-2 text-right text-xs">{formatMoney(d.familyDeduction)}</td>}
                                {visibleColumns.tax_income_total && <td className="border border-gray-300 text-right px-2 bg-gray-50">{formatMoney(taxableIncomeTotal)}</td>}
                                {visibleColumns.tax_assessable && <td className="border border-gray-300 text-right px-2 bg-gray-50 font-bold">{formatMoney(assessableIncome)}</td>}
                                ...
                                {/* Company Cost */}
                                {visibleColumns.comp_bhxh && <td className="border border-gray-300 text-right px-2 text-xs text-gray-500">{d.isContractSigned ? formatMoney(compBhxh) : '-'}</td>}
                                {visibleColumns.comp_bhyt && <td className="border border-gray-300 text-right px-2 text-xs text-gray-500">{d.isContractSigned ? formatMoney(compBhyt) : '-'}</td>}
                                {visibleColumns.comp_bhtn && <td className="border border-gray-300 text-right px-2 text-xs text-gray-500">{d.isContractSigned ? formatMoney(compBhtn) : '-'}</td>}
                                {visibleColumns.comp_union && <td className="border border-gray-300 px-2 text-right text-xs">{d.isContractSigned ? formatMoney(effectiveUnionFee) : '-'}</td>}
                                
                                {visibleColumns.holiday_bonus && <td className="border border-gray-300 px-2 text-right">{formatMoney(d.holidayBonus)}</td>}
                ...
                <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl relative z-10 overflow-hidden animate-in zoom-in duration-200">
                    <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                        <h3 className="font-bold text-lg text-gray-800 flex items-center">
                            <DollarSign className="mr-2 text-indigo-600" /> Cấu hình Thu nhập
                        </h3>
                    ...
                    <div className="p-6 space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <select 
                                    ...
                                    value={newMoneyConfig.type}
                                    onChange={(e) => {
                                        const type = e.target.value as MoneyConfigType;
                                        let name = '';
                                        if (type === 'parking') name = 'Phụ cấp gửi xe';
                                        if (type === 'bonus') name = 'Thưởng';
                                        if (type === 'salary13') name = 'Lương tháng 13';
                                        if (type === 'holidayBonus') name = 'Thưởng lễ';
                                        setNewMoneyConfig({ ...newMoneyConfig, type, name });
                                    }}
                                >
                            ...
                            <div>
                                ...
                                <input 
                                    type="text" className="w-full border rounded-lg p-2 text-sm font-bold"
                                    value={newMoneyConfig.name}
                                    onChange={(e) => setNewMoneyConfig({...newMoneyConfig, name: e.target.value})}
                                />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input 
                                type="number" className="w-full border rounded-lg p-2 text-lg font-bold text-indigo-600 outline-none focus:border-indigo-500"
                                value={newMoneyConfig.amount}
                                onChange={(e) => setNewMoneyConfig({...newMoneyConfig, amount: Number(e.target.value)})}
                            />
                        </div>
                ...
                <div className="bg-white rounded-3xl w-full max-w-7xl max-h-[85vh] overflow-y-auto shadow-2xl animate-in zoom-in duration-200 border border-gray-100" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="p-5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex justify-between items-center sticky top-0 z-10 shadow-md">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-xl font-bold">{users.find(u => u.id === editingMember)?.name}</h3>
                    ...
                    <div className="p-5 bg-gray-50">
                        {/* 
                           Updated Grid: Use 'items-stretch' to make columns equal height.
                           Inner containers use 'flex flex-col h-full' and content use 'flex-1' to stretch.
                        */}
